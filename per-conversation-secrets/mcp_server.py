...
class MCPHandler(BaseHTTPRequestHandler):
    ...
    def _handle_sse_connection(self) -> None:
        ...
        try:
            ...
            endpoint_event = self._send_sse_event(
    ...
    def do_POST(self) -> None:
        ...
        auth_header = self.headers.get("Authorization", "none")[:30]
        logger.info(f"  Headers: Authorization={auth_header}...")
        ...
        if method == "initialize":
            return {
                ...
                "result": {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "token-validator", "version": "1.0.0"},
                },
            }
                    ...
                    "tools": [
                        {
                            "name": "validate_token",
                            "description": (
                                "Validates that the secret token was correctly "
                                "passed to this MCP server via config expansion"
                            ),
                            "inputSchema": {
                                "type": "object",
                                "properties": {
                                    "echo_message": {
                                        "type": "string",
                                        "description": "Message to echo on success",
                                    }
                                },
                                "required": [],
                            },
            ...
            if tool_name == "validate_token":
                ...
                logger.info(
                    f"Validating token: '{token}' vs expected: '{EXPECTED_TOKEN}'"
                )

                if token == EXPECTED_TOKEN:
                    VALIDATED_COUNT += 1
                    result_text = (
                        f"✅ SUCCESS! Token validated correctly. "
                        f"Validation count: {VALIDATED_COUNT}"
                    )
                    if echo_msg:
                        result_text += f"\nEcho: {echo_msg}"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="MCP Token Validator Server with SSE support"
    )
    parser.add_argument("--port", type=int, default=12000, help="Port to listen on")
    parser.add_argument(
        "--expected-token", type=str, required=True, help="Expected token value"
    )
    ...
    logger.info("  GET  /mcp    - SSE event stream")
