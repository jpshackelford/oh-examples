...
if [ -z "$path" ]; then
cat <<EOF
{"decision": "deny", "reason": "🚧 WORKSPACE ISOLATION: file_editor command $cmd requires a path."}
EOF
exit 2
fi
...
*)
cat <<EOF
{"decision": "deny", "reason": "🚧 WORKSPACE ISOLATION: file_editor $cmd blocked - path $path is outside your workspace ($workspace). Use a path inside the workspace, or use view for read-only access."}
EOF
exit 2
;;
esac
...
../*|*/../*|*/..)
cat <<EOF
{"decision": "deny", "reason": "🚧 WORKSPACE ISOLATION: file_editor $cmd blocked - path $path uses parent-directory (..) traversal which may escape the workspace."}
EOF
exit 2
;;
esac
