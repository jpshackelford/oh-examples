...
def create_conversation(title: str = "Test Conversation for Archive Demo") -> Dict[str, Any]:
    """
...
def main():
    ...
    response = input(f"Archive conversation {conversation_id}? This will delete the sandbox and release its PVC. (yes/no): ")
    if response.lower() != "yes":
        print()
        print("Cancelled. Conversation will remain active.")
        print(f"To archive it later, run: python archive_sandbox.py archive {conversation_id}")
        return
    ...
    print("3. Use 'python archive_sandbox.py cleanup' to bulk-archive stopped conversations")
    print()
